...
# convert table and child elements to divs
...
@func XMLNode.wrap_children_sections(Text %separator, Text %wrapper, Text %class) {
  # overloaded function to wrap groups of children into specific child "section"
  # elements. parameters:
  # %separator: XPath string for the first element of how the groups are
  #   currently separated. no pipes allowed.
  # %wrapper: string for an element name to wrap around the children
  # %class: string for classname for element
  $(%separator) {
    wrap(%wrapper, class: %class)
  }
  $("./*[not(self::" + %wrapper + "[contains(@class, '" + %class + "')])]") {
    move_to("preceding-sibling::" + %wrapper + "[contains(@class, '" + %class + "')][1]")
  }
}
@func XMLNode.wrap_children_sections(Text %separator, Text %wrapper, Text %class, Text %between_bool) {
  # %between_bool: boolean (only excepts "true" string) to determine whether
  #   sections can only be determined by separators in between "sections"
  match(%between_bool) {
    with("true") {
      $(%separator + "[1]") { 
        dup() {
          attribute("id", "mw-wrap-children-section-helper")
          move_to("..", "top")
        }
      }
    }
  }
  wrap_children_sections(%separator, %wrapper, %class)
  match(%between_bool) {
    with("true") {
      $("./" + %wrapper + "[contains(@class, '" + %class + "')]") {
        remove(%separator)
      }
    }
  }
